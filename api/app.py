...
Эндпоинты (за прокси видны как /v2/api/...):
  ...
  PUT    /upload?name=<имя>&ext=glb|stl|splat|ply|ksplat → тело = сырой файл;
         валидация: glb — магия 'glTF'; stl — бинарная структура/ASCII 'solid';
         ply — магия 'ply'; splat/ksplat — только токен и размер (магии нет)
...
class H(BaseHTTPRequestHandler):
    ...
    def do_PUT(self):
        ...
        if ext not in ("glb", "stl", "splat", "ply", "ksplat"):
            self._drain(length)
            return self._json(400, {"error": "ext должен быть glb|stl|splat|ply|ksplat"})
    ...
    def do_POST(self):
        ...
        q = parse_qs(u.query)
        rel = q.get("file", [""])[0]
        if not re.fullmatch(r"uploads/[A-Za-z0-9_.-]+\.(glb|stl|splat|ply|ksplat)", rel) or "/../" in rel:
        ...
        with LOCK:
            lst = load_models()
            hit = [m for m in lst if rel in (m.get("glb"), m.get("stl"), m.get("splat"), m.get("ply"), m.get("ksplat"))]
            if not hit:
    ...
    def do_DELETE(self):
        ...
        with LOCK:
            lst = load_models()
            keep = [m for m in lst
                    if rel not in (m.get("glb"), m.get("stl"), m.get("splat"), m.get("ply"), m.get("ksplat"))]
            if len(keep) == len(lst):
                return self._json(404, {"error": "нет в списке"})
            save_models(keep)
            try:
