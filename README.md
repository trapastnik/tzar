...
| `/v2/` | **v2 — единый вьюер: меши + гауссианы в одном списке.** Меши (.glb/.stl): режимы Текстура / Геометрия / Wireframe (цвет линий) / Высота / Уклон / Нормали / Детали / AO / Кривизна, сечения, сравнение сканов. Сплаты (.ply/.splat/.ksplat/.spz, GS3D DropInViewer в общей сцене): та же камера, повороты, список. Загрузка любых форматов прямо с сайта; SPZ v3 (Scaniverse) конвертируется в браузере автоматически | `v2/index.html`, `v2/*.glb`, `v2/uploads/` (runtime) |
| `/splats/` | Запасной «чистый» сплат-вьюер (тот же функционал по сплатам, без меш-кода) | `splats/index.html` |
